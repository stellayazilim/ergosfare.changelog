    ...
    ) : ICommandHandler<JoinUserToRoleCommand> 
{
    ...
    async Task HandleAsync(JoinUserToRoleCommand command, IExecutionContext context ) 
    {
        ...
        var role = await snapshotService.Snapshot("snapshot-role-exist", new Snapshot<RoleEntity>(() => 
        {
            return await roleRepository.GetRoleByNameAsync(command.Role.Name, context.CancellationToken)        
        })); 
        
        comamand.User.AddRole(role)
        
        
        await snapshotService.Snapshot("snapshot-add-user-to-role-snapshot", new Snapshot(() => 
        {
            return await userRepository.UpdateUserAsync(command.User, context.CancellationToken); 
        })); 
    } 
}
```
